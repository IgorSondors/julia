...
def BRA_conv_conv(input):
    bn = BatchNormalization()(input)
    activation = Activation('swish')(bn)
    pooling = AveragePooling2D(pool_size=(2, 2), strides=(2, 2))(activation)

    conv01 = Conv2D(32, (3, 3), use_bias=False)(pooling)
...
def build_shared_plain_network(input_image, mode, using_SE=True):
    ...
    block1 = BRA_conv_conv(input=conv1)#BRA(input=conv1)
...
if __name__ == '__main__':
    ...
    initial_lr = 0.0017
    min_lr = 0.0000001
    dst_pth = '/home/yandex/igor/julia/conv_after__pooling/conv_conv'
