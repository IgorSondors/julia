...
def dct_2d(
    ...
    return tf.cast(X2_t, tf.float16)
        
def white_norm(input):
    return (input - tf.constant(127.5, dtype=tf.float16)) / 128.0
...
def fft3d_function(x, dtype = "complex64"):
    ...
    real_x_f, imag_x_f = tf.math.real(x_f), tf.math.imag(x_f)
    return tf.cast(real_x_f, tf.float16), tf.cast(imag_x_f, tf.float16)
...
if __name__ == '__main__':
    ...
    factor = 0.9
    patience = 1
    initial_lr = 2.8*10**(-5)#0.0017
    min_lr = 0.0000001
    dst_pth = '/home/yandex/igor/julia/dct_bigger_batch_mixed_float16/wn'
    csv_train = '/mnt/data/lossless_train_04102022_crops.csv'
    csv_test = '/mnt/data/lossless_val_04102022_crops.csv'

    #model = create_model(img_size)
    wght_pth = '/home/yandex/igor/julia/dct_bigger_batch_mixed_float16/wn/c3ae-128-epoch:030-val_loss:0.0940-val_accuracy:0.9639.h5'
